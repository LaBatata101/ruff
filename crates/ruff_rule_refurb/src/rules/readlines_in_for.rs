use ruff_linter_settings::preview::is_readlines_in_for_fix_safe;
...
/// FURB129
pub fn readlines_in_for(checker: &CheckerSnapshot, for_stmt: &StmtFor) {
    readlines_in_iter(checker, for_stmt.iter.as_ref());
}

/// FURB129
pub fn readlines_in_comprehension(checker: &CheckerSnapshot, comprehension: &Comprehension) {
    readlines_in_iter(checker, &comprehension.iter);
}

fn readlines_in_iter(checker: &CheckerSnapshot, iter_expr: &Expr) {
    ...
    } else if !is_io_base_expr(expr_attr.value.as_ref(), &semantic) {
        return;
    }
