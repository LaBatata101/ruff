...
use crate::helpers::replace_with_identity_check;
...
/// FURB169
pub fn type_none_comparison(checker: &CheckerSnapshot, compare: &ast::ExprCompare) {
    ...
    let Some(left_arg) = type_call_arg(&compare.left, &semantic) else {
        return;
    };
    let Some(right_arg) = type_call_arg(right, &semantic) else {
        return;
    };
