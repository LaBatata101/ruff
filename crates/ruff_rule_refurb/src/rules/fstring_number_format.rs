...
/// FURB116
pub fn fstring_number_format(checker: &CheckerSnapshot, subscript: &ast::ExprSubscript) {
    ...
    let Some(id) = semantic.resolve_builtin_symbol(func) else {
        return;
    };
...
fn try_create_replacement(checker: &CheckerSnapshot, arg: &Expr, base: Base) -> Option<String> {
    if !matches!(
        arg,
