...
use crate::helpers::LoggerCandidateVisitor;
...
/// TRY400
pub fn error_instead_of_exception(checker: &CheckerSnapshot, handlers: &[ExceptHandler]) {
    let semantic = checker.semantic();
    for handler in handlers {
        let ExceptHandler::ExceptHandler(ast::ExceptHandlerExceptHandler { body, .. }) = handler;
        let calls = {
            let mut visitor =
                LoggerCandidateVisitor::new(&semantic, &checker.settings.logger_objects);
            visitor.visit_body(body);
            visitor.calls
        };
        for (expr, logging_level) in calls {
            if matches!(logging_level, LoggingLevel::Error) && exc_info(&expr.arguments, &checker.semantic()).is_none() {
