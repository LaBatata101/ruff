...
use ruff_linter_commons::docstrings::sections::{SectionContext, SectionContexts, SectionKind};
use ruff_linter_commons::docstrings::styles::SectionStyle;
use ruff_linter_commons::docstrings::Docstring;
use ruff_codes::Rule;
use crate::helpers::find_underline;
...
fn blanks_and_section_underline(
    ...
    if let Some(non_blank_line) = following_lines.next() {
        if let Some(dashed_line) = find_underline(&non_blank_line, '-') {
            if num_blank_lines_after_header > 0 && checker.enabled(Rule::MissingSectionUnderlineAfterName) {
            ...
            if dashed_line.len().to_usize() != context.section_name().len() && checker.enabled(Rule::MismatchedSectionUnderlineLength) {
                let mut diagnostic = Diagnostic::new(
            ...
            } else if checker.enabled(Rule::EmptyDocstringSection) {
                checker.report_diagnostic(Diagnostic::new(
                    EmptyDocstringSection {
                        name: context.section_name().to_string(),
                    },
            ...
            if num_blank_lines_after_header > 0 && checker.enabled(Rule::BlankLinesBetweenHeaderAndContent) {
                // If the section is followed by exactly one line, and then a
                // reStructuredText directive, the blank lines should be preserved, as in:
                //
                // ```python
...
fn common_section(
    ...
    if checker.enabled(Rule::NoBlankLineBeforeSection) && !context
            .previous_line()
...
fn missing_args(checker: &CheckerSnapshot, docstring: &Docstring, docstrings_args: &FxHashSet<String>) {
...
fn parameters_section(checker: &CheckerSnapshot, docstring: &Docstring, context: &SectionContext) {
...
fn numpy_section(
    ...
    if checker.enabled(Rule::UndocumentedParam) && matches!(context.kind(), SectionKind::Parameters) {
