...
use ruff_linter_fix::edits::adjust_indentation;
use ruff_linter_settings::preview::is_only_add_return_none_at_end_enabled;
use ruff_codes::{AsRule, Rule};
use crate::helpers::end_of_last_statement;
...
/// RET501
fn unnecessary_return_none(checker: &CheckerSnapshot, decorator_list: &[Decorator], stack: &Stack) {
...
/// RET502
fn implicit_return_value(checker: &CheckerSnapshot, stack: &Stack) {
    for stmt in &stack.returns {
...
fn add_return_none(checker: &CheckerSnapshot, stmt: &Stmt, range: TextRange) {
...
fn implicit_returns<'a>(checker: &CheckerSnapshot, stmt: &'a Stmt) -> Vec<&'a Stmt> {
    match stmt {
        Stmt::If(ast::StmtIf {
            body,
            elif_else_clauses,
...
/// RET503
fn implicit_return(checker: &CheckerSnapshot, function_def: &ast::StmtFunctionDef, stmt: &Stmt) {
...
/// RET504
fn unnecessary_assign(checker: &CheckerSnapshot, stack: &Stack) {
...
/// RET505, RET506, RET507, RET508
fn superfluous_elif_else(checker: &CheckerSnapshot, stack: &Stack) {
...
pub fn function(checker: &CheckerSnapshot, function_def: &ast::StmtFunctionDef) {
    ...
    let stack = {
        let mut visitor = ReturnVisitor::new(&semantic);
        for stmt in body {
    ...
    } else if checker.enabled(Rule::UnnecessaryReturnNone) {
