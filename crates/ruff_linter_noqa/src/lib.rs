...
use ruff_codes::{AsRule, Rule, RuleSet, NoqaCode};
...
impl Codes<'_> {
    ...
    pub fn iter(&self) -> std::slice::Iter<Code> {
        self.codes.iter()
    }

    /// Returns `true` if the string list of `codes` includes `code` (or an alias
    /// thereof).
    pub fn includes(&self, needle: Rule) -> bool {
...
impl FileExemption<'_> {
    ...
    /// exemption.
    pub fn enumerates(&self, needle: Rule) -> bool {
        let needle = needle.noqa_code();
...
#[derive(Debug)]
pub struct FileNoqaDirectives<'a>(Vec<FileNoqaDirectiveLine<'a>>);
...
impl<'a> FileNoqaDirectives<'a> {
    ...
    pub fn lines(&self) -> &[FileNoqaDirectiveLine] {
        &self.0
    }
}
...
pub fn lex_codes(text: &str) -> Result<Vec<Code<'_>>, LexicalError> {
...
#[derive(Debug)]
pub struct NoqaDirectiveLine<'a> {
    /// The range of the text line for which the noqa directive applies.
    pub range: TextRange,
    /// The noqa directive.
...
pub struct NoqaDirectives<'a> {
    inner: Vec<NoqaDirectiveLine<'a>>,
}

impl<'a> NoqaDirectives<'a> {
    pub fn from_commented_ranges(
        comment_ranges: &CommentRanges,
        external: &[String],
        path: &Path,
        locator: &'a Locator<'a>,
                ...
                Ok(Some(NoqaLexerOutput {
                    ...
                    if let Directive::Codes(codes) = &directive {
                        ...
                        for code in &codes.codes {
                            ...
                            if !external
                                .iter()
                                .any(|external| code.as_str().starts_with(external)) && Rule::from_code(
...
//     };
//     use ruff_codes::rules::pycodestyle::{AmbiguousVariableName, UselessSemicolon};
//     use ruff_codes::rules::pyflakes::UnusedVariable;
//     use ruff_codes::rules::pyupgrade::PrintfStringFormatting;
