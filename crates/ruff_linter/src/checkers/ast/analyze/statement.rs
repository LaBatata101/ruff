...
use ruff_rule_fastapi::{self as fastapi};
...
use ruff_rule_flake8_errmsg::{self as flake8_errmsg};
...
use ruff_rule_flake8_raise::{self as flake8_raise};
use ruff_rule_flake8_return::{self as flake8_return};
use ruff_rule_flake8_simplify::{self as flake8_simplify};
use ruff_rule_flake8_slots::{self as flake8_slots};
...
use ruff_rule_tryceratops::{self as tryceratops};
    ...
    match stmt {
        Stmt::Global(ast::StmtGlobal { names, range: _ }) => {
            if checker.enabled(Rule::GlobalAtModuleLevel) {
                pylint::rules::global_at_module_level(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::AmbiguousVariableName) {
                for name in names {
                    pycodestyle::rules::ambiguous_variable_name(&checker.snapshot(), name, name.range());
                }
            }
        }
        Stmt::Nonlocal(nonlocal @ ast::StmtNonlocal { names, range: _ }) => {
            ...
            if checker.enabled(Rule::NonlocalAndGlobal) {
                pylint::rules::nonlocal_and_global(&checker.snapshot(), nonlocal);
            }
        }
        Stmt::Break(_) => {
        ...
        Stmt::FunctionDef(
            ...
            if checker.enabled(Rule::DjangoNonLeadingReceiverDecorator) {
                flake8_django::rules::non_leading_receiver_decorator(&checker.snapshot(), decorator_list);
            }
            if checker.enabled(Rule::FastApiRedundantResponseModel) {
                fastapi::rules::fastapi_redundant_response_model(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::FastApiNonAnnotatedDependency) {
                fastapi::rules::fastapi_non_annotated_dependency(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::FastApiUnusedPathParameter) {
                fastapi::rules::fastapi_unused_path_parameter(&checker.snapshot(), function_def);
            }
            ...
            if checker.enabled(Rule::InvalidBoolReturnType) {
                pylint::rules::invalid_bool_return(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::InvalidLengthReturnType) {
                pylint::rules::invalid_length_return(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::InvalidBytesReturnType) {
                pylint::rules::invalid_bytes_return(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::InvalidIndexReturnType) {
                pylint::rules::invalid_index_return(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::InvalidHashReturnType) {
                pylint::rules::invalid_hash_return(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::InvalidStrReturnType) {
                pylint::rules::invalid_str_return(&checker.snapshot(), function_def);
            }
            ...
            if checker.source_type.is_stub() {
                if checker.enabled(Rule::PassStatementStubBody) {
                    flake8_pyi::rules::pass_statement_stub_body(&checker.snapshot(), body);
                }
                if checker.enabled(Rule::NonEmptyStubBody) {
                    flake8_pyi::rules::non_empty_stub_body(&checker.snapshot(), body);
                }
                if checker.enabled(Rule::StubBodyMultipleStatements) {
                    flake8_pyi::rules::stub_body_multiple_statements(&checker.snapshot(), stmt, body);
                }
            }
            if checker.enabled(Rule::AnyEqNeAnnotation) {
                flake8_pyi::rules::any_eq_ne_annotation(&checker.snapshot(), name, parameters);
            }
            ...
            if checker.enabled(Rule::GeneratorReturnFromIterMethod) {
                flake8_pyi::rules::bad_generator_return_type(function_def, &checker.snapshot());
            }
            if checker.source_type.is_stub() {
                if checker.enabled(Rule::StrOrReprDefinedInStub) {
                    flake8_pyi::rules::str_or_repr_defined_in_stub(&checker.snapshot(), stmt);
                }
            }
            if checker.source_type.is_stub() || checker.target_version() >= PythonVersion::PY311 {
                if checker.enabled(Rule::NoReturnArgumentAnnotationInStub) {
                    flake8_pyi::rules::no_return_argument_annotation(&checker.snapshot(), parameters);
                }
            }
            if checker.enabled(Rule::BadExitAnnotation) {
                flake8_pyi::rules::bad_exit_annotation(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::RedundantNumericUnion) {
                flake8_pyi::rules::redundant_numeric_union(&checker.snapshot(), parameters);
            }
            if checker.enabled(Rule::Pep484StylePositionalOnlyParameter) {
                flake8_pyi::rules::pep_484_positional_parameter(&checker.snapshot(), function_def);
            }
            ...
            if checker.enabled(Rule::GlobalStatement) {
                pylint::rules::global_statement(&checker.snapshot(), name);
            }
            if checker.enabled(Rule::LRUCacheWithoutParameters) {
                if checker.target_version() >= PythonVersion::PY38 {
                    pyupgrade::rules::lru_cache_without_parameters(&checker.snapshot(), decorator_list);
                }
            }
            if checker.enabled(Rule::LRUCacheWithMaxsizeNone) {
                if checker.target_version() >= PythonVersion::PY39 {
                    pyupgrade::rules::lru_cache_with_maxsize_none(&checker.snapshot(), decorator_list);
                }
            }
            if checker.enabled(Rule::CachedInstanceMethod) {
                flake8_bugbear::rules::cached_instance_method(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::MutableArgumentDefault) {
                flake8_bugbear::rules::mutable_argument_default(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::ReturnInGenerator) {
                flake8_bugbear::rules::return_in_generator(&checker.snapshot(), function_def);
            }
            if checker.any_enabled(&[
                ...
                flake8_return::rules::function(&checker.snapshot(), function_def);
            }
            ...
            if checker.enabled(Rule::HardcodedPasswordDefault) {
                flake8_bandit::rules::hardcoded_password_default(&checker.snapshot(), parameters);
            }
            if checker.enabled(Rule::SuspiciousMarkSafeUsage) {
                for decorator in decorator_list {
                    flake8_bandit::rules::suspicious_function_decorator(&checker.snapshot(), decorator);
                }
            }
            if checker.enabled(Rule::PropertyWithParameters) {
                pylint::rules::property_with_parameters(&checker.snapshot(), stmt, decorator_list, parameters);
            }
            if checker.enabled(Rule::TooManyArguments) {
                pylint::rules::too_many_arguments(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::TooManyPositionalArguments) {
                pylint::rules::too_many_positional_arguments(&checker.snapshot(), function_def);
            }
            ...
            if checker.any_enabled(&[
                ...
                flake8_pytest_style::rules::marks(&checker.snapshot(), decorator_list);
            }
            ...
            if checker.enabled(Rule::FStringDocstring) {
                flake8_bugbear::rules::f_string_docstring(&checker.snapshot(), body);
            }
            ...
            if checker.enabled(Rule::AsyncFunctionWithTimeout) {
                flake8_async::rules::async_function_with_timeout(&checker.snapshot(), function_def);
            }
            #[cfg(any(feature = "test-rules", test))]
            if checker.enabled(Rule::UnreachableCode) {
                pylint::rules::in_function(&checker.snapshot(), name, body);
            }
            if checker.enabled(Rule::ReimplementedOperator) {
                refurb::rules::reimplemented_operator(&checker.snapshot(), &function_def.into());
            }
            if checker.enabled(Rule::SslWithBadDefaults) {
                flake8_bandit::rules::ssl_with_bad_defaults(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::UnusedAsync) {
                ruff::rules::unused_async(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::WhitespaceAfterDecorator) {
                pycodestyle::rules::whitespace_after_decorator(&checker.snapshot(), decorator_list);
            }
            if checker.enabled(Rule::PostInitDefault) {
                ruff::rules::post_init_default(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::PytestParameterWithDefaultArgument) {
                flake8_pytest_style::rules::parameter_with_default_argument(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::Airflow3Removal) {
                airflow::rules::airflow_3_removal_function_def(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::NonPEP695GenericFunction) {
                pyupgrade::rules::non_pep695_generic_function(&checker.snapshot(), function_def);
            }
            if checker.enabled(Rule::InvalidArgumentName) {
                pep8_naming::rules::invalid_argument_name_function(&checker.snapshot(), function_def);
            }
        }
        Stmt::Return(_) => {
            if checker.enabled(Rule::ReturnInInit) {
                pylint::rules::return_in_init(&checker.snapshot(), stmt);
            }
        }
        Stmt::ClassDef(
            ...
            if checker.enabled(Rule::NoClassmethodDecorator) {
                pylint::rules::no_classmethod_decorator(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::NoStaticmethodDecorator) {
                pylint::rules::no_staticmethod_decorator(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::DjangoNullableModelStringField) {
                flake8_django::rules::nullable_model_string_field(&checker.snapshot(), body);
            }
            if checker.enabled(Rule::DjangoExcludeWithModelForm) {
                flake8_django::rules::exclude_with_model_form(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::DjangoAllWithModelForm) {
                flake8_django::rules::all_with_model_form(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::DjangoUnorderedBodyContentInModel) {
                flake8_django::rules::unordered_body_content_in_model(&checker.snapshot(), class_def);
            }
            if !checker.source_type.is_stub() {
                if checker.enabled(Rule::DjangoModelWithoutDunderStr) {
                    flake8_django::rules::model_without_dunder_str(&checker.snapshot(), class_def);
                }
            }
            if checker.enabled(Rule::EqWithoutHash) {
                pylint::rules::object_without_hash_method(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::ClassAsDataStructure) {
                flake8_bugbear::rules::class_as_data_structure(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::RedefinedSlotsInSubclass) {
                pylint::rules::redefined_slots_in_subclass(&checker.snapshot(), class_def);
            }
            ...
            if checker.enabled(Rule::UselessObjectInheritance) {
                pyupgrade::rules::useless_object_inheritance(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::ReplaceStrEnum) {
                if checker.target_version() >= PythonVersion::PY311 {
                    pyupgrade::rules::replace_str_enum(&checker.snapshot(), class_def);
                }
            }
            if checker.enabled(Rule::UnnecessaryClassParentheses) {
                pyupgrade::rules::unnecessary_class_parentheses(&checker.snapshot(), class_def);
            }
            ...
            if checker.source_type.is_stub() {
                ...
                if checker.enabled(Rule::PassInClassBody) {
                    flake8_pyi::rules::pass_in_class_body(&checker.snapshot(), class_def);
                }
            }
            if checker.enabled(Rule::EllipsisInNonEmptyClassBody) {
                flake8_pyi::rules::ellipsis_in_non_empty_class_body(&checker.snapshot(), body);
            }
            if checker.enabled(Rule::GenericNotLastBaseClass) {
                flake8_pyi::rules::generic_not_last_base_class(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::PytestIncorrectMarkParenthesesStyle) {
                flake8_pytest_style::rules::marks(&checker.snapshot(), decorator_list);
            }
            if checker.enabled(Rule::DuplicateClassFieldDefinition) {
                flake8_pie::rules::duplicate_class_field_definition(&checker.snapshot(), body);
            }
            if checker.enabled(Rule::NonUniqueEnums) {
                flake8_pie::rules::non_unique_enums(&checker.snapshot(), stmt, body);
            }
            if checker.enabled(Rule::FStringDocstring) {
                flake8_bugbear::rules::f_string_docstring(&checker.snapshot(), body);
            }
            ...
            if checker.enabled(Rule::DuplicateBases) {
                pylint::rules::duplicate_bases(&checker.snapshot(), name, arguments.as_deref());
            }
            if checker.enabled(Rule::NoSlotsInStrSubclass) {
                flake8_slots::rules::no_slots_in_str_subclass(&checker.snapshot(), stmt, class_def);
            }
            if checker.enabled(Rule::NoSlotsInTupleSubclass) {
                flake8_slots::rules::no_slots_in_tuple_subclass(&checker.snapshot(), stmt, class_def);
            }
            if checker.enabled(Rule::NoSlotsInNamedtupleSubclass) {
                flake8_slots::rules::no_slots_in_namedtuple_subclass(&checker.snapshot(), stmt, class_def);
            }
            if checker.enabled(Rule::NonSlotAssignment) {
                pylint::rules::non_slot_assignment(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::SingleStringSlots) {
                pylint::rules::single_string_slots(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::MetaClassABCMeta) {
                refurb::rules::metaclass_abcmeta(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::WhitespaceAfterDecorator) {
                pycodestyle::rules::whitespace_after_decorator(&checker.snapshot(), decorator_list);
            }
            if checker.enabled(Rule::SubclassBuiltin) {
                refurb::rules::subclass_builtin(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::DataclassEnum) {
                ruff::rules::dataclass_enum(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::NonPEP695GenericClass) {
                pyupgrade::rules::non_pep695_generic_class(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::ClassWithMixedTypeVars) {
                ruff::rules::class_with_mixed_type_vars(&checker.snapshot(), class_def);
            }
            if checker.enabled(Rule::ImplicitClassVarInDataclass) {
                ruff::rules::implicit_class_var_in_dataclass(&checker.snapshot(), class_def);
            }
        }
        Stmt::Import(ast::StmtImport { names, range: _ }) => {
            if checker.enabled(Rule::MultipleImportsOnOneLine) {
                pycodestyle::rules::multiple_imports_on_one_line(&checker.snapshot(), stmt, names);
            }
            if checker.enabled(Rule::ModuleImportNotAtTopOfFile) {
                pycodestyle::rules::module_import_not_at_top_of_file(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::ImportOutsideTopLevel) {
                pylint::rules::import_outside_top_level(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::GlobalStatement) {
                for name in names {
                    if let Some(asname) = name.asname.as_ref() {
                        pylint::rules::global_statement(&checker.snapshot(), asname);
                    } else {
                        pylint::rules::global_statement(&checker.snapshot(), &name.name);
                    }
                }
            }
            if checker.enabled(Rule::DeprecatedCElementTree) {
                pyupgrade::rules::deprecated_c_element_tree(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::DeprecatedMockImport) {
                pyupgrade::rules::deprecated_mock_import(&checker.snapshot(), stmt);
            }
            if checker.any_enabled(&[
                ...
                flake8_bandit::rules::suspicious_imports(&checker.snapshot(), stmt);
            }

            if checker.enabled(Rule::BannedModuleLevelImports) {
                flake8_tidy_imports::rules::banned_module_level_imports(&checker.snapshot(), stmt);
            }

            for alias in names {
                if checker.enabled(Rule::NonAsciiImportName) {
                    pylint::rules::non_ascii_module_import(&checker.snapshot(), alias);
                }
                ...
                if checker.enabled(Rule::BannedApi) {
                    flake8_tidy_imports::rules::banned_api(
                        &checker.snapshot(),
                        &ruff_linter_commons::matchers::NameMatchPolicy::MatchNameOrParent(
                            ruff_linter_commons::matchers::MatchNameOrParent {
                                module: &alias.name,
                            },
                        ),
                        &alias,
                ...
                if !checker.source_type.is_stub() {
                    if checker.enabled(Rule::UselessImportAlias) {
                        pylint::rules::useless_import_alias(&checker.snapshot(), alias);
                    }
                }
                if checker.enabled(Rule::ManualFromImport) {
                    pylint::rules::manual_from_import(&checker.snapshot(), stmt, alias, names);
                }
                ...
                if let Some(asname) = &alias.asname {
                    ...
                    if checker.enabled(Rule::CamelcaseImportedAsAcronym) {
                        if let Some(diagnostic) = pep8_naming::rules::camelcase_imported_as_acronym(
                            name, asname, alias, stmt, &checker.snapshot(),
                        ) {
                            checker.report_diagnostic(diagnostic);
                        }
                    }
                ...
                if checker.enabled(Rule::BuiltinImportShadowing) {
                    flake8_builtins::rules::builtin_import_shadowing(&checker.snapshot(), alias);
                }
            }
        }
        Stmt::ImportFrom(
            ...
            if checker.enabled(Rule::UnnecessaryFutureImport) {
                if checker.target_version() >= PythonVersion::PY37 {
                    if let Some("__future__") = module {
                        pyupgrade::rules::unnecessary_future_import(&checker.snapshot(), stmt, names);
                    }
                }
            }
            if checker.enabled(Rule::DeprecatedMockImport) {
            ...
            if checker.enabled(Rule::DeprecatedImport) {
                pyupgrade::rules::deprecated_import(&checker.snapshot(), import_from);
            }
            if checker.enabled(Rule::UnnecessaryBuiltinImport) {
                if let Some(module) = module {
                    pyupgrade::rules::unnecessary_builtin_import(&checker.snapshot(), stmt, module, names);
                }
            }
            if checker.any_enabled(&[
                Rule::SuspiciousTelnetlibImport,
            ...
            if checker.enabled(Rule::BannedApi) {
                if let Some(module) = helpers::resolve_imported_module_path(
                    ...
                    flake8_tidy_imports::rules::banned_api(
                        &checker.snapshot(),
                        &ruff_linter_commons::matchers::NameMatchPolicy::MatchNameOrParent(
                            ruff_linter_commons::matchers::MatchNameOrParent { module: &module },
                        ),
                        &stmt,
                    );
                    ...
                    for alias in names {
                        ...
                        flake8_tidy_imports::rules::banned_api(
                            &checker.snapshot(),
                            &ruff_linter_commons::matchers::NameMatchPolicy::MatchName(
                                ruff_linter_commons::matchers::MatchName {
                                    module: &module,
                                    member: &alias.name,
                                },
                            ),
            ...
            if checker.source_type.is_stub() {
                if checker.enabled(Rule::FutureAnnotationsInStub) {
                    flake8_pyi::rules::from_future_import(&checker.snapshot(), import_from);
                }
            }
            for alias in names {
                if let Some("__future__") = module {
                    if checker.enabled(Rule::FutureFeatureNotDefined) {
                        pyflakes::rules::future_feature_not_defined(&checker.snapshot(), alias);
                    }
                } else if &alias.name == "*" {
                ...
                if let Some(asname) = &alias.asname {
                    ...
                    if !checker.source_type.is_stub() {
                        if checker.enabled(Rule::UselessImportAlias) {
                            pylint::rules::useless_import_from_alias(&checker.snapshot(), alias, module, level);
                        }
                    }
                }
                if checker.enabled(Rule::BuiltinImportShadowing) {
            ...
            if checker.enabled(Rule::ByteStringUsage) {
                flake8_pyi::rules::bytestring_import(&checker.snapshot(), import_from);
            }
        }
        Stmt::Raise(raise @ ast::StmtRaise { exc, .. }) => {
            if checker.enabled(Rule::RaiseNotImplemented) {
                if let Some(expr) = exc {
                    pyflakes::rules::raise_not_implemented(&checker.snapshot(), expr);
                }
            }
            if checker.enabled(Rule::RaiseLiteral) {
                if let Some(exc) = exc {
                    flake8_bugbear::rules::raise_literal(&checker.snapshot(), exc);
                }
            }
            if checker.any_enabled(&[
                ...
                if let Some(exc) = exc {
                    flake8_errmsg::rules::string_in_exception(&checker.snapshot(), stmt, exc);
                }
            }
            if checker.enabled(Rule::OSErrorAlias) {
                if let Some(item) = exc {
                    pyupgrade::rules::os_error_alias_raise(&checker.snapshot(), item);
                }
            }
            if checker.enabled(Rule::TimeoutErrorAlias) {
                if checker.target_version() >= PythonVersion::PY310 {
                    if let Some(item) = exc {
                        pyupgrade::rules::timeout_error_alias_raise(&checker.snapshot(), item);
                    }
                }
            }
            if checker.enabled(Rule::RaiseVanillaClass) {
                if let Some(expr) = exc {
                    tryceratops::rules::raise_vanilla_class(&checker.snapshot(), expr);
                }
            }
            if checker.enabled(Rule::RaiseVanillaArgs) {
                if let Some(expr) = exc {
                    tryceratops::rules::raise_vanilla_args(&checker.snapshot(), expr);
                }
            }
            if checker.enabled(Rule::UnnecessaryParenOnRaiseException) {
                if let Some(expr) = exc {
                    flake8_raise::rules::unnecessary_paren_on_raise_exception(&checker.snapshot(), expr);
                }
            }
            if checker.enabled(Rule::MisplacedBareRaise) {
                pylint::rules::misplaced_bare_raise(&checker.snapshot(), raise);
            }
        }
        Stmt::AugAssign(aug_assign @ ast::StmtAugAssign { target, .. }) => {
            if checker.enabled(Rule::GlobalStatement) {
                if let Expr::Name(ast::ExprName { id, .. }) = target.as_ref() {
                    pylint::rules::global_statement(&checker.snapshot(), id);
                }
            }
            if checker.enabled(Rule::UnsortedDunderAll) {
                ruff::rules::sort_dunder_all_aug_assign(&checker.snapshot(), aug_assign);
            }
        }
        Stmt::If(
            if_ @ ast::StmtIf {
            ...
            if checker.enabled(Rule::TooManyNestedBlocks) {
                pylint::rules::too_many_nested_blocks(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::EmptyTypeCheckingBlock) {
                flake8_type_checking::rules::empty_type_checking_block(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::IfTuple) {
                pyflakes::rules::if_tuple(&checker.snapshot(), if_);
            }
            ...
            if checker.enabled(Rule::IfWithSameArms) {
                flake8_simplify::rules::if_with_same_arms(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::NeedlessBool) {
                flake8_simplify::rules::needless_bool(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::IfElseBlockInsteadOfDictLookup) {
                flake8_simplify::rules::if_else_block_instead_of_dict_lookup(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::IfElseBlockInsteadOfIfExp) {
                flake8_simplify::rules::if_else_block_instead_of_if_exp(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::IfElseBlockInsteadOfDictGet) {
                flake8_simplify::rules::if_else_block_instead_of_dict_get(&checker.snapshot(), if_);
            }
            ...
            if checker.enabled(Rule::OutdatedVersionBlock) {
                pyupgrade::rules::outdated_version_block(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::CollapsibleElseIf) {
                pylint::rules::collapsible_else_if(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::CheckAndRemoveFromSet) {
                refurb::rules::check_and_remove_from_set(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::SliceToRemovePrefixOrSuffix) {
                refurb::rules::slice_to_remove_affix_stmt(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::TooManyBooleanExpressions) {
                pylint::rules::too_many_boolean_expressions(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::IfStmtMinMax) {
                pylint::rules::if_stmt_min_max(&checker.snapshot(), if_);
            }
            if checker.source_type.is_stub() {
                if checker.any_enabled(&[
                    ...
                    if let Expr::BoolOp(ast::ExprBoolOp { values, .. }) = test.as_ref() {
                        for value in values {
                            flake8_pyi::rules::unrecognized_version_info(&checker.snapshot(), value);
                        }
                    } else {
                        flake8_pyi::rules::unrecognized_version_info(&checker.snapshot(), test);
                    }
                }
                if checker.any_enabled(&[
                    ...
                    if let Expr::BoolOp(ast::ExprBoolOp { values, .. }) = test.as_ref() {
                        for value in values {
                            flake8_pyi::rules::unrecognized_platform(&checker.snapshot(), value);
                        }
                    } else {
                        flake8_pyi::rules::unrecognized_platform(&checker.snapshot(), test);
                    }
                }
                if checker.enabled(Rule::ComplexIfStatementInStub) {
                    if let Expr::BoolOp(ast::ExprBoolOp { values, .. }) = test.as_ref() {
                        for value in values {
                            flake8_pyi::rules::complex_if_statement_in_stub(&checker.snapshot(), value);
                        }
                    } else {
                        flake8_pyi::rules::complex_if_statement_in_stub(&checker.snapshot(), test);
                    }
                }
            }
            ...
            if checker.enabled(Rule::IfKeyInDictDel) {
                ruff::rules::if_key_in_dict_del(&checker.snapshot(), if_);
            }
            if checker.enabled(Rule::NeedlessElse) {
                ruff::rules::needless_else(&checker.snapshot(), if_.into());
            }
        }
        Stmt::Assert(
            ...
            if checker.enabled(Rule::AssertTuple) {
                pyflakes::rules::assert_tuple(&checker.snapshot(), stmt, test);
            }
            if checker.enabled(Rule::AssertFalse) {
                flake8_bugbear::rules::assert_false(&checker.snapshot(), stmt, test, msg.as_deref());
            }
            if checker.enabled(Rule::PytestAssertAlwaysFalse) {
                flake8_pytest_style::rules::assert_falsy(&checker.snapshot(), stmt, test);
            }
            ...
            if checker.enabled(Rule::AssertOnStringLiteral) {
                pylint::rules::assert_on_string_literal(&checker.snapshot(), test);
            }
            if checker.enabled(Rule::InvalidMockAccess) {
                pygrep_hooks::rules::non_existent_mock_method(&checker.snapshot(), test);
            }
            if checker.enabled(Rule::AssertWithPrintMessage) {
                ruff::rules::assert_with_print_message(&checker.snapshot(), assert_stmt);
            }
            if checker.enabled(Rule::InvalidAssertMessageLiteralArgument) {
                ruff::rules::invalid_assert_message_literal_argument(&checker.snapshot(), assert_stmt);
            }
        }
        Stmt::With(with_stmt @ ast::StmtWith { items, body, .. }) => {
            ...
            if checker.enabled(Rule::AssertRaisesException) {
                flake8_bugbear::rules::assert_raises_exception(&checker.snapshot(), items);
            }
            if checker.enabled(Rule::PytestRaisesWithMultipleStatements) {
                flake8_pytest_style::rules::complex_raises(&checker.snapshot(), stmt, items, body);
            }
            if checker.enabled(Rule::PytestWarnsWithMultipleStatements) {
                flake8_pytest_style::rules::complex_warns(&checker.snapshot(), stmt, items, body);
            }
            ...
            if checker.enabled(Rule::RedefinedLoopName) {
                pylint::rules::redefined_loop_name(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::ReadWholeFile) {
                refurb::rules::read_whole_file(&checker.snapshot(), with_stmt);
            }
            if checker.enabled(Rule::WriteWholeFile) {
                refurb::rules::write_whole_file(&checker.snapshot(), with_stmt);
            }
            if checker.enabled(Rule::UselessWithLock) {
                pylint::rules::useless_with_lock(&checker.snapshot(), with_stmt);
            }
            if checker.enabled(Rule::CancelScopeNoCheckpoint) {
                flake8_async::rules::cancel_scope_no_checkpoint(&checker.snapshot(), with_stmt, items);
            }
        }
        Stmt::While(while_stmt @ ast::StmtWhile { body, orelse, .. }) => {
            ...
            if checker.enabled(Rule::FunctionUsesLoopVariable) {
                flake8_bugbear::rules::function_uses_loop_variable(&checker.snapshot(), &Node::Stmt(stmt));
            }
            if checker.enabled(Rule::UselessElseOnLoop) {
                pylint::rules::useless_else_on_loop(&checker.snapshot(), stmt, body, orelse);
            }
            if checker.enabled(Rule::TryExceptInLoop) {
                perflint::rules::try_except_in_loop(&checker.snapshot(), body);
            }
            if checker.enabled(Rule::AsyncBusyWait) {
                flake8_async::rules::async_busy_wait(&checker.snapshot(), while_stmt);
            }
            if checker.enabled(Rule::NeedlessElse) {
                ruff::rules::needless_else(&checker.snapshot(), while_stmt.into());
            }
        }
        Stmt::For(
            ...
            if checker.any_enabled(&[
                ...
                let snapshot = checker.semantic().snapshot();
                checker.analyze.for_loops.push(snapshot);
            }
            if checker.enabled(Rule::LoopVariableOverridesIterator) {
                flake8_bugbear::rules::loop_variable_overrides_iterator(&checker.snapshot(), target, iter);
            }
            if checker.enabled(Rule::FunctionUsesLoopVariable) {
                flake8_bugbear::rules::function_uses_loop_variable(&checker.snapshot(), &Node::Stmt(stmt));
            }
            if checker.enabled(Rule::ReuseOfGroupbyGenerator) {
                flake8_bugbear::rules::reuse_of_groupby_generator(&checker.snapshot(), target, body, iter);
            }
            if checker.enabled(Rule::UselessElseOnLoop) {
                pylint::rules::useless_else_on_loop(&checker.snapshot(), stmt, body, orelse);
            }
            ...
            if checker.enabled(Rule::IterationOverSet) {
                pylint::rules::iteration_over_set(&checker.snapshot(), iter);
            }
            if checker.enabled(Rule::DictIterMissingItems) {
                pylint::rules::dict_iter_missing_items(&checker.snapshot(), target, iter);
            }
            if checker.enabled(Rule::ManualListCopy) {
                perflint::rules::manual_list_copy(&checker.snapshot(), for_stmt);
            }

            if checker.enabled(Rule::ModifiedIteratingSet) {
                pylint::rules::modified_iterating_set(&checker.snapshot(), for_stmt);
            }
            if checker.enabled(Rule::UnnecessaryListCast) {
                perflint::rules::unnecessary_list_cast(&checker.snapshot(), iter, body);
            }
            if checker.enabled(Rule::UnnecessaryListIndexLookup) {
                pylint::rules::unnecessary_list_index_lookup(&checker.snapshot(), for_stmt);
            }
            if checker.enabled(Rule::UnnecessaryDictIndexLookup) {
                pylint::rules::unnecessary_dict_index_lookup(&checker.snapshot(), for_stmt);
            }
            if checker.enabled(Rule::ReadlinesInFor) {
                refurb::rules::readlines_in_for(&checker.snapshot(), for_stmt);
            }
            if !*is_async {
                if checker.enabled(Rule::ReimplementedBuiltin) {
                    flake8_simplify::rules::convert_for_loop_to_any_all(&checker.snapshot(), stmt);
                }
                if checker.enabled(Rule::InDictKeys) {
                    flake8_simplify::rules::key_in_dict_for(&checker.snapshot(), for_stmt);
                }
                if checker.enabled(Rule::TryExceptInLoop) {
                    perflint::rules::try_except_in_loop(&checker.snapshot(), body);
                }
                if checker.enabled(Rule::ForLoopSetMutations) {
                    refurb::rules::for_loop_set_mutations(&checker.snapshot(), for_stmt);
                }
                if checker.enabled(Rule::ForLoopWrites) {
                    refurb::rules::for_loop_writes_stmt(&checker.snapshot(), for_stmt);
                }
            }
            if checker.enabled(Rule::NeedlessElse) {
                ruff::rules::needless_else(&checker.snapshot(), for_stmt.into());
            }
        }
        Stmt::Try(
            ...
            if checker.enabled(Rule::JumpStatementInFinally) {
                flake8_bugbear::rules::jump_statement_in_finally(&checker.snapshot(), finalbody);
            }
            if checker.enabled(Rule::ContinueInFinally) {
                if checker.target_version() <= PythonVersion::PY38 {
                    pylint::rules::continue_in_finally(&checker.snapshot(), finalbody);
                }
            }
            ...
            if checker.any_enabled(&[
                ...
                flake8_bugbear::rules::duplicate_exceptions(&checker.snapshot(), handlers);
            }
            if checker.enabled(Rule::RedundantTupleInExceptionHandler) {
                flake8_bugbear::rules::redundant_tuple_in_exception_handler(&checker.snapshot(), handlers);
            }
            if checker.enabled(Rule::OSErrorAlias) {
                pyupgrade::rules::os_error_alias_handlers(&checker.snapshot(), handlers);
            }
            if checker.enabled(Rule::TimeoutErrorAlias) {
                if checker.target_version() >= PythonVersion::PY310 {
                    pyupgrade::rules::timeout_error_alias_handlers(&checker.snapshot(), handlers);
                }
            }
            if checker.enabled(Rule::PytestAssertInExcept) {
                flake8_pytest_style::rules::assert_in_exception_handler(&checker.snapshot(), handlers);
            }
            if checker.enabled(Rule::SuppressibleException) {
                flake8_simplify::rules::suppressible_exception(
                    &checker.snapshot(), stmt, body, handlers, orelse, finalbody,
                );
            }
            if checker.enabled(Rule::ReturnInTryExceptFinally) {
                flake8_simplify::rules::return_in_try_except_finally(
                    &checker.snapshot(), body, handlers, finalbody,
                );
            }
            if checker.enabled(Rule::TryConsiderElse) {
                tryceratops::rules::try_consider_else(&checker.snapshot(), body, orelse, handlers);
            }
            if checker.enabled(Rule::VerboseRaise) {
                tryceratops::rules::verbose_raise(&checker.snapshot(), handlers);
            }
            if checker.enabled(Rule::VerboseLogMessage) {
                tryceratops::rules::verbose_log_message(&checker.snapshot(), handlers);
            }
            if checker.enabled(Rule::RaiseWithinTry) {
                tryceratops::rules::raise_within_try(&checker.snapshot(), body, handlers);
            }
            if checker.enabled(Rule::UselessTryExcept) {
                tryceratops::rules::useless_try_except(&checker.snapshot(), handlers);
            }
            if checker.enabled(Rule::ErrorInsteadOfException) {
                tryceratops::rules::error_instead_of_exception(&checker.snapshot(), handlers);
            }
            if checker.enabled(Rule::NeedlessElse) {
                ruff::rules::needless_else(&checker.snapshot(), try_stmt.into());
            }
        }
        Stmt::Assign(assign @ ast::StmtAssign { targets, value, .. }) => {
            if checker.enabled(Rule::SelfOrClsAssignment) {
                for target in targets {
                    pylint::rules::self_or_cls_assignment(&checker.snapshot(), target);
                }
            }
            if checker.enabled(Rule::RedeclaredAssignedName) {
                pylint::rules::redeclared_assigned_name(&checker.snapshot(), targets);
            }
            if checker.enabled(Rule::LambdaAssignment) {
                if let [target] = &targets[..] {
                    pycodestyle::rules::lambda_assignment(&checker.snapshot(), target, value, None, stmt);
                }
            }
            if checker.enabled(Rule::AssignmentToOsEnviron) {
                flake8_bugbear::rules::assignment_to_os_environ(&checker.snapshot(), targets);
            }
            if checker.enabled(Rule::HardcodedPasswordString) {
                flake8_bandit::rules::assign_hardcoded_password_string(&checker.snapshot(), value, targets);
            }
            if checker.enabled(Rule::GlobalStatement) {
                for target in targets {
            ...
            {
                airflow::rules::variable_name_task_id(&checker.snapshot(), targets, value);
            }
            if checker.settings.rules.enabled(Rule::SelfAssigningVariable) {
                pylint::rules::self_assignment(&checker.snapshot(), assign);
            }
            if checker.settings.rules.enabled(Rule::TypeParamNameMismatch) {
                pylint::rules::type_param_name_mismatch(&checker.snapshot(), value, targets);
            }
            if checker
                .settings
                .rules
            ...
            {
                pylint::rules::type_name_incorrect_variance(&checker.snapshot(), value);
            }
            if checker.settings.rules.enabled(Rule::TypeBivariance) {
                pylint::rules::type_bivariance(&checker.snapshot(), value);
            }
            if checker.enabled(Rule::NonAugmentedAssignment) {
                pylint::rules::non_augmented_assignment(&checker.snapshot(), assign);
            }
            if checker.settings.rules.enabled(Rule::UnsortedDunderAll) {
                ruff::rules::sort_dunder_all_assign(&checker.snapshot(), assign);
            }
            if checker.source_type.is_stub() {
                if checker.any_enabled(&[
            ...
            if checker.enabled(Rule::ListReverseCopy) {
                refurb::rules::list_assign_reversed(&checker.snapshot(), assign);
            }
            if checker.enabled(Rule::NonPEP695TypeAlias) {
                pyupgrade::rules::non_pep695_type_alias_type(&checker.snapshot(), assign);
            }
        }
        Stmt::AnnAssign(
            ...
            if checker.enabled(Rule::SelfAssigningVariable) {
                pylint::rules::self_annotated_assignment(&checker.snapshot(), assign_stmt);
            }
            ...
            if checker.enabled(Rule::NonPEP695TypeAlias) {
                pyupgrade::rules::non_pep695_type_alias(&checker.snapshot(), assign_stmt);
            }
            if checker.enabled(Rule::HardcodedPasswordString) {
            ...
            if checker.settings.rules.enabled(Rule::UnsortedDunderAll) {
                ruff::rules::sort_dunder_all_ann_assign(&checker.snapshot(), assign_stmt);
            }
            if checker.source_type.is_stub() {
                if let Some(value) = value {
                    if checker.enabled(Rule::AssignmentDefaultInStub) {
                            ...
                            .any(|scope| scope.kind.is_function())
                        {
                            flake8_pyi::rules::annotated_assignment_default_in_stub(
                                &checker.snapshot(), target, value, annotation,
                            );
                        }
                    }
                } else {
            ...
            if checker.semantic().match_typing_expr(annotation, "TypeAlias") {
                if checker.enabled(Rule::SnakeCaseTypeAlias) {
                    flake8_pyi::rules::snake_case_type_alias(&checker.snapshot(), target);
                }
                if checker.enabled(Rule::TSuffixedTypeAlias) {
                    flake8_pyi::rules::t_suffixed_type_alias(&checker.snapshot(), target);
                }
            } else if checker
                .semantic()
                .match_typing_expr(helpers::map_subscript(annotation), "Final")
            {
                if checker.enabled(Rule::RedundantFinalLiteral) {
                    flake8_pyi::rules::redundant_final_literal(&checker.snapshot(), assign_stmt);
                }
            }
        }
        Stmt::TypeAlias(ast::StmtTypeAlias { name, .. }) => {
            if checker.enabled(Rule::SnakeCaseTypeAlias) {
                flake8_pyi::rules::snake_case_type_alias(&checker.snapshot(), name);
            }
            if checker.enabled(Rule::TSuffixedTypeAlias) {
                flake8_pyi::rules::t_suffixed_type_alias(&checker.snapshot(), name);
            }
        }
        Stmt::Delete(delete @ ast::StmtDelete { targets, range: _ }) => {
            ...
            if checker.enabled(Rule::DeleteFullSlice) {
                refurb::rules::delete_full_slice(&checker.snapshot(), delete);
            }
        }
        Stmt::Expr(expr @ ast::StmtExpr { value, range: _ }) => {
            if checker.enabled(Rule::UselessComparison) {
                flake8_bugbear::rules::useless_comparison(&checker.snapshot(), value);
            }
            if checker.enabled(Rule::UselessExpression) {
                flake8_bugbear::rules::useless_expression(&checker.snapshot(), value);
            }
            if checker.enabled(Rule::InvalidMockAccess) {
                pygrep_hooks::rules::uncalled_mock_method(&checker.snapshot(), value);
            }
            if checker.enabled(Rule::NamedExprWithoutContext) {
                pylint::rules::named_expr_without_context(&checker.snapshot(), value);
            }
            ...
            if checker.enabled(Rule::RepeatedAppend) {
                refurb::rules::repeated_append(&checker.snapshot(), stmt);
            }
            if checker.enabled(Rule::UselessExceptionStatement) {
                pylint::rules::useless_exception_statement(&checker.snapshot(), expr);
            }
        }
        Stmt::Match(ast::StmtMatch {
            ...
            if checker.enabled(Rule::NanComparison) {
                pylint::rules::nan_comparison_match(&checker.snapshot(), cases);
            }
        }
        _ => {}
    }
