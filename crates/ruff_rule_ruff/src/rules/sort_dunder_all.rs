...
/// For example: `__all__ += ["b", "c", "a"]`.
pub fn sort_dunder_all_aug_assign(checker: &CheckerSnapshot, node: &ast::StmtAugAssign) {
    if node.op.is_add() {
...
pub fn sort_dunder_all_extend_call(
    checker: &CheckerSnapshot,
    ast::ExprCall {
        func,
...
pub fn sort_dunder_all_ann_assign(checker: &CheckerSnapshot, node: &ast::StmtAnnAssign) {
...
fn sort_dunder_all(checker: &CheckerSnapshot, target: &ast::Expr, node: &ast::Expr) {
