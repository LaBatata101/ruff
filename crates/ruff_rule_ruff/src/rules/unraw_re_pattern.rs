...
/// RUF039
pub fn unraw_re_pattern(checker: &CheckerSnapshot, call: &ExprCall) {
...
fn check_string(checker: &CheckerSnapshot, literal: &StringLiteral, module: RegexModule, func: &str) {
...
fn check_bytes(checker: &CheckerSnapshot, literal: &BytesLiteral, module: RegexModule, func: &str) {
    if literal.flags.prefix().is_raw() {
        return;
    }
