...
/// SIM113
pub fn enumerate_for_loop(checker: &CheckerSnapshot, for_stmt: &ast::StmtFor) {
    ...
    for stmt in &for_stmt.body {
        ...
        if let Some(index) = match_index_increment(stmt) {
            ...
            let Some(id) = semantic.resolve_name(index) else {
                continue;
            };
            ...
            let Some(value) = typing::find_binding_value(binding, &semantic) else {
                continue;
            };
            if !matches!(
                value,
