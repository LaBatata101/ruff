...
pub fn is_same_expr<'a>(a: &'a Expr, b: &'a Expr) -> Option<&'a str> {
...
/// SIM101
pub fn duplicate_isinstance_call(checker: &CheckerSnapshot, expr: &Expr) {
    ...
    for (index, call) in values.iter().enumerate() {
        let Some(target) = isinstance_target(call, &semantic) else {
...
/// SIM109
pub fn compare_with_tuple(checker: &CheckerSnapshot, expr: &Expr) {
    let Expr::BoolOp(ast::ExprBoolOp {
        op: BoolOp::Or,
        values,
        range: _,
...
/// SIM220
pub fn expr_and_not_expr(checker: &CheckerSnapshot, expr: &Expr) {
    let Expr::BoolOp(ast::ExprBoolOp {
        op: BoolOp::And,
        values,
        range: _,
...
/// SIM221
pub fn expr_or_not_expr(checker: &CheckerSnapshot, expr: &Expr) {
    let Expr::BoolOp(ast::ExprBoolOp {
        op: BoolOp::Or,
        values,
        range: _,
...
/// SIM222
pub fn expr_or_true(checker: &CheckerSnapshot, expr: &Expr) {
...
/// SIM223
pub fn expr_and_false(checker: &CheckerSnapshot, expr: &Expr) {
    if checker.semantic().in_string_type_definition() {
        return;
    }
