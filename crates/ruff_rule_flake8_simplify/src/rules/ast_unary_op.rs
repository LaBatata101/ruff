...
/// SIM201
pub fn negation_with_equal_op(checker: &CheckerSnapshot, expr: &Expr, op: UnaryOp, operand: &Expr) {
...
/// SIM208
pub fn double_negation(checker: &CheckerSnapshot, expr: &Expr, op: UnaryOp, operand: &Expr) {
    if !matches!(op, UnaryOp::Not) {
        return;
    }
