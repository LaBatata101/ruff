...
use ruff_linter_fix::cst::helpers::{negate, space};
use ruff_linter_fix::codemods::CodegenStylist;
use ruff_linter_fix::edits::pad;
use crate::rules::ObjectType;
use ruff_linter_commons::Locator;
use ruff_linter_checkers::ast::CheckerSnapshot;
use ruff_linter_fix::cst::matchers::{
...
/// range(3)}`.
pub(crate) fn fix_unnecessary_generator_dict(expr: &Expr, checker: &CheckerSnapshot) -> Result<Edit> {
...
pub(crate) fn fix_unnecessary_literal_dict(expr: &Expr, checker: &CheckerSnapshot) -> Result<Edit> {
    let locator = checker.locator();
    let stylist = checker.stylist();
        ...
        if outer_name.value == "list" {
            ...
            let args = if inner_call.args.iter().any(|arg| {
                ...
                if let [arg] = args.as_mut_slice() {
                    if matches!(arg.value, Expression::GeneratorExp(_)) && arg.value.lpar().is_empty() && arg.value.rpar().is_empty() {
                        arg.value = arg
                            .value
                            .clone()
...
pub(crate) fn fix_unnecessary_map(
    ...
    // syntax errors.
    if matches!(object_type, ObjectType::Set | ObjectType::Dict) && parent.is_some_and(Expr::is_f_string_expr) {
