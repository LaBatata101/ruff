...
use crate::fixes::{pad_end, pad_start};
...
/// C401 (`set(generator)`)
pub fn unnecessary_generator_set(checker: &CheckerSnapshot, call: &ast::ExprCall) {
    ...
    if let [generator] = generators.as_slice() {
        if generator.ifs.is_empty() && !generator.is_async && ComparableExpr::from(elt) == ComparableExpr::from(&generator.target) {
