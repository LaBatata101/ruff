...
/// PYI061
pub fn redundant_none_literal<'a>(checker: &CheckerSnapshot, literal_expr: &'a Expr) {
    ...
    for none_expr in none_exprs {
        ...
        diagnostic.try_set_optional_fix(|| {
            ...
            .map(|fix| {
                fix.map(|fix| fix.isolate(CheckerSnapshot::isolation(semantic.current_statement_id())))
            })
        });
