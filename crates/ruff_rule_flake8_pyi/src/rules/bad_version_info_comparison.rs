...
use ruff_linter_settings::preview::is_bad_version_info_in_non_stub_enabled;
use ruff_codes::Rule;
...
/// PYI006, PYI066
pub fn bad_version_info_comparison(checker: &CheckerSnapshot, test: &Expr, has_else_clause: bool) {
    ...
    if matches!(op, CmpOp::Lt) {
        if checker.enabled(Rule::BadVersionInfoOrder) && (checker.source_type.is_stub() || is_bad_version_info_in_non_stub_enabled(checker.settings)) && has_else_clause {
            checker.report_diagnostic(Diagnostic::new(BadVersionInfoOrder, test.range()));
        }
    } else if checker.enabled(Rule::BadVersionInfoComparison) {
