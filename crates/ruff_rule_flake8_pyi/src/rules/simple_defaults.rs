...
fn is_final_assignment(annotation: &Expr, value: &Expr, semantic: &SemanticModel) -> bool {
    if matches!(value, Expr::Name(_) | Expr::Attribute(_)) && semantic.match_typing_expr(annotation, "Final") {
        return true;
    }
    false
}
...
/// PYI011
pub fn typed_argument_simple_defaults(checker: &CheckerSnapshot, parameters: &Parameters) {
    for parameter in parameters.iter_non_variadic_params() {
        let Some(default) = parameter.default() else {
            continue;
        };
        if parameter.annotation().is_some() && !is_valid_default_value_with_annotation(
                default,
                true,
                checker.locator(),
                &checker.semantic(),
...
/// PYI014
pub fn argument_simple_defaults(checker: &CheckerSnapshot, parameters: &Parameters) {
    for parameter in parameters.iter_non_variadic_params() {
        let Some(default) = parameter.default() else {
            continue;
        };
        if parameter.annotation().is_none() && !is_valid_default_value_with_annotation(
                default,
                true,
                checker.locator(),
                &checker.semantic(),
...
/// PYI015
pub fn assignment_default_in_stub(checker: &CheckerSnapshot, targets: &[Expr], value: &Expr) {
...
/// PYI052
pub fn unannotated_assignment_in_stub(checker: &CheckerSnapshot, targets: &[Expr], value: &Expr) {
    let [target] = targets else {
        return;
    };
...
/// PYI035
pub fn unassigned_special_variable_in_stub(checker: &CheckerSnapshot, target: &Expr, stmt: &Stmt) {
    let Expr::Name(ast::ExprName { id, .. }) = target else {
        return;
    };
...
/// PYI026
pub fn type_alias_without_annotation(checker: &CheckerSnapshot, value: &Expr, targets: &[Expr]) {
    let [target] = targets else {
        return;
    };
