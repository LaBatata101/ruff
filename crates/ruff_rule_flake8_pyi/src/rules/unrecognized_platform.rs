...
/// PYI007, PYI008
pub fn unrecognized_platform(checker: &CheckerSnapshot, test: &Expr) {
    ...
    if let Expr::StringLiteral(ast::ExprStringLiteral { value, .. }) = right {
        ...
        if checker.enabled(Rule::UnrecognizedPlatformName) && !matches!(value.to_str(), "linux" | "win32" | "cygwin" | "darwin") {
