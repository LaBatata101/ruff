...
/// PYI041
pub fn redundant_numeric_union(checker: &CheckerSnapshot, parameters: &Parameters) {
    for annotation in parameters.iter().filter_map(AnyParameterRef::annotation) {
        check_annotation(checker, annotation);
    }
}

fn check_annotation<'a>(checker: &CheckerSnapshot, annotation: &'a Expr) {
    let mut numeric_flags = NumericFlags::empty();

    let semantic = checker.semantic();
    let mut find_numeric_type = |expr: &Expr, _parent: &Expr| {
        let Some(builtin_type) = semantic.resolve_builtin_symbol(expr) else {
            return;
        };
