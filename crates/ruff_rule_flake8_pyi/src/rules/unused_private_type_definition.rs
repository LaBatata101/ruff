...
/// PYI018
pub fn unused_private_type_var(checker: &CheckerSnapshot, scope: &Scope) {
...
/// PYI046
pub fn unused_private_protocol(checker: &CheckerSnapshot, scope: &Scope) {
    let semantic = checker.semantic();
    for binding in scope
        .binding_ids()
        .map(|binding_id| semantic.binding(binding_id))
...
/// PYI047
pub fn unused_private_type_alias(checker: &CheckerSnapshot, scope: &Scope) {
    let semantic = checker.semantic();
...
/// PYI049
pub fn unused_private_typed_dict(checker: &CheckerSnapshot, scope: &Scope) {
    let semantic = checker.semantic();
