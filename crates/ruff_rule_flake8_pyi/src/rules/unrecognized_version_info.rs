...
pub fn unrecognized_version_info(checker: &CheckerSnapshot, test: &Expr) {
...
fn version_check(
    ...
    if expected == ExpectedComparator::MajorDigit {
        if !is_int_constant(comparator) && checker.enabled(Rule::UnrecognizedVersionInfoCheck) {
            checker
    ...
    if checker.enabled(Rule::WrongTupleLengthVersionComparison) && (op == CmpOp::Eq || op == CmpOp::NotEq) {
