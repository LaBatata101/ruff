...
use ruff_linter_fix::importer::ImportedMembers;
use crate::helpers::{filter_contained, quote_annotation};
...
/// TC004
pub fn runtime_import_in_type_checking_block(checker: &CheckerSnapshot, scope: &Scope) {
...
fn quote_imports(checker: &CheckerSnapshot, node_id: NodeId, imports: &[ImportBinding]) -> Fix {
    ...
    Fix::unsafe_edits(head, rest).isolate(CheckerSnapshot::isolation(
        checker.semantic().parent_statement_id(node_id),
    ))
}

/// Generate a [`Fix`] to remove runtime imports from a type-checking block.
fn move_imports(checker: &CheckerSnapshot, node_id: NodeId, imports: &[ImportBinding]) -> Result<Fix> {
    ...
    Ok(
        Fix::unsafe_edits(remove_import_edit, add_import_edit.into_edits()).isolate(
            CheckerSnapshot::isolation(checker.semantic().parent_statement_id(node_id)),
        ),
    )
}
