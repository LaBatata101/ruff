...
/// B023
pub fn function_uses_loop_variable(checker: &CheckerSnapshot, node: &Node) {
    ...
    if !suspicious_variables.is_empty() {
        ...
        for name in suspicious_variables {
            if reassigned_in_loop.contains(&name.id.as_str()) && checker.insert_flake8_bugbear_range(name.range()) {
