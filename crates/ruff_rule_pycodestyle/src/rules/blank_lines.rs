...
use ruff_linter_checkers::logical_lines::expand_indent;
use ruff_linter_commons::line_width::IndentWidth;
use crate::helpers::is_non_logical_token;
...
#[derive(Debug)]
pub struct BlankLinesChecker<'a> {
    stylist: &'a Stylist<'a>,
    locator: &'a Locator<'a>,
    indent_width: IndentWidth,
...
impl<'a> BlankLinesChecker<'a> {
    pub fn new(
        locator: &'a Locator<'a>,
        stylist: &'a Stylist<'a>,
        settings: &ruff_linter_settings::LinterSettings,
        source_type: PySourceType,
        cell_offsets: Option<&'a CellOffsets>,
    ) -> BlankLinesChecker<'a> {
        BlankLinesChecker {
    ...
    pub fn check_lines(&self, tokens: &Tokens, diagnostics: &mut Vec<Diagnostic>) {
        ...
        for logical_line in line_preprocessor {
            ...
            if let Some(prev_logical_line) = prev_logical_line {
                if prev_logical_line.is_comment_only && prev_logical_line.indent_length != logical_line.indent_length {
