...
/// T201, T203
pub fn print_call(checker: &CheckerSnapshot, call: &ast::ExprCall) {
    ...
    let mut diagnostic = match qualified_name.segments() {
        ["" | "builtins", "print"] => {
            ...
            if let Some(keyword) = call.arguments.find_keyword("file") {
                if !keyword.value.is_none_literal_expr() && semantic.resolve_qualified_name(&keyword.value).is_none_or(
                        |qualified_name| {
    ...
    if semantic.current_expression_parent().is_none() {
        ...
        diagnostic.set_fix(
            Fix::unsafe_edit(edit)
                .isolate(CheckerSnapshot::isolation(semantic.current_statement_parent_id())),
        );
    }
