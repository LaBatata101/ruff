...
/// PLW0127
pub fn self_or_cls_assignment(checker: &CheckerSnapshot, target: &Expr) {
    ...
    let Some(parent) = &semantic.first_non_type_parent_scope(checker.semantic().current_scope())
    else {
        return;
    };
...
fn check_expr(checker: &CheckerSnapshot, target: &Expr, method_type: MethodType) {
    match target {
        Expr::Name(_) => {
