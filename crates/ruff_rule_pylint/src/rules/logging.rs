...
/// PLE1205
/// PLE1206
pub fn logging_call(checker: &CheckerSnapshot, call: &ast::ExprCall) {
    ...
    if checker.enabled(Rule::LoggingTooManyArgs) && summary.num_positional < num_message_args {
        checker.report_diagnostic(Diagnostic::new(LoggingTooManyArgs, call.func.range()));
    }

    if checker.enabled(Rule::LoggingTooFewArgs) && num_message_args > 0 && num_keywords == 0 && summary.num_positional > num_message_args {
