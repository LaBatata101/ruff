...
/// PLW0177
pub fn nan_comparison(checker: &CheckerSnapshot, left: &Expr, comparators: &[Expr]) {
    nan_comparison_impl(checker, std::iter::once(left).chain(comparators));
}

/// PLW0177
pub fn nan_comparison_match(checker: &CheckerSnapshot, cases: &[ast::MatchCase]) {
    nan_comparison_impl(
        checker,
        cases
            .iter()
...
fn nan_comparison_impl<'a>(checker: &CheckerSnapshot, comparators: impl Iterator<Item = &'a Expr>) {
