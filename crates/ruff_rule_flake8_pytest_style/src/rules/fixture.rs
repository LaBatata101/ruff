...
/// PT001, PT002, PT003
fn check_fixture_decorator(checker: &CheckerSnapshot, func_name: &str, decorator: &Decorator) {
    match &decorator.expression {
        Expr::Call(ast::ExprCall {
            ...
            if checker.enabled(Rule::PytestFixtureIncorrectParenthesesStyle) && !checker.settings.flake8_pytest_style.fixture_parentheses
            ...
            if checker.enabled(Rule::PytestFixturePositionalArgs) && !arguments.args.is_empty() {
...
/// PT022
fn check_fixture_returns(checker: &CheckerSnapshot, name: &str, body: &[Stmt], returns: Option<&Expr>) {
...
/// PT019
fn check_test_function_args(checker: &CheckerSnapshot, parameters: &Parameters, decorators: &[Decorator]) {
...
/// PT020
fn check_fixture_decorator_name(checker: &CheckerSnapshot, decorator: &Decorator) {
...
/// PT021
fn check_fixture_addfinalizer(checker: &CheckerSnapshot, parameters: &Parameters, body: &[Stmt]) {
...
/// PT024, PT025
fn check_fixture_marks(checker: &CheckerSnapshot, decorators: &[Decorator]) {
    for (expr, marker) in get_mark_decorators(decorators) {
        if checker.enabled(Rule::PytestUnnecessaryAsyncioMarkOnFixture) && marker == "asyncio" {
            let mut diagnostic =
        ...
        if checker.enabled(Rule::PytestErroneousUseFixturesOnFixture) && marker == "usefixtures" {
            let mut diagnostic =
