...
pub fn is_pytest_warns(func: &Expr, semantic: &SemanticModel) -> bool {
    semantic
...
/// PT029, PT030
pub fn warns_call(checker: &CheckerSnapshot, call: &ast::ExprCall) {
    if is_pytest_warns(&call.func, &checker.semantic()) {
        if checker.enabled(Rule::PytestWarnsWithoutWarning) && call.arguments.is_empty() {
...
/// PT031
pub fn complex_warns(checker: &CheckerSnapshot, stmt: &Stmt, items: &[WithItem], body: &[Stmt]) {
...
/// PT030
fn warning_needs_match(checker: &CheckerSnapshot, warning: &Expr) {
