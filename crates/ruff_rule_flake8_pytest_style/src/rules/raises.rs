...
use ruff_linter_commons::helpers::is_pytest_raises;
...
pub fn raises_call(checker: &CheckerSnapshot, call: &ast::ExprCall) {
    if is_pytest_raises(&call.func, &checker.semantic()) {
        if checker.enabled(Rule::PytestRaisesWithoutException) && call
                .arguments
...
pub fn complex_raises(checker: &CheckerSnapshot, stmt: &Stmt, items: &[WithItem], body: &[Stmt]) {
...
/// PT011
fn exception_needs_match(checker: &CheckerSnapshot, exception: &Expr) {
