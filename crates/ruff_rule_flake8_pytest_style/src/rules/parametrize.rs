...
/// PT006
fn check_names(checker: &CheckerSnapshot, call: &ExprCall, expr: &Expr, argvalues: &Expr) {
...
/// PT007
fn check_values(checker: &CheckerSnapshot, names: &Expr, values: &Expr) {
...
/// PT014
fn check_duplicates(checker: &CheckerSnapshot, values: &Expr) {
...
fn handle_single_name(checker: &CheckerSnapshot, argnames: &Expr, value: &Expr, argvalues: &Expr) {
    let mut diagnostic = Diagnostic::new(
        PytestParametrizeNamesWrongType {
...
fn unpack_single_element_items(checker: &CheckerSnapshot, expr: &Expr) -> Vec<Edit> {
...
pub fn parametrize(checker: &CheckerSnapshot, call: &ExprCall) {
