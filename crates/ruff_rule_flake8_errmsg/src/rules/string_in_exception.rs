...
/// EM101, EM102, EM103
pub fn string_in_exception(checker: &CheckerSnapshot, stmt: &Stmt, exc: &Expr) {
        ...
        if let Some(first) = args.first() {
            match first {
                ...
                Expr::StringLiteral(ast::ExprStringLiteral { value: string, .. }) => {
                    if checker.enabled(Rule::RawStringInException) && string.len() >= checker.settings.flake8_errmsg.max_string_length {
                        let mut diagnostic =
