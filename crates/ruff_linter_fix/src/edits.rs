...
use ruff_linter_commons::line_width::{IndentWidth, LineLength, LineWidthBuilder};
...
pub fn delete_stmt(
    ...
    } else if let Some(semicolon) = trailing_semicolon(stmt.end(), locator) {
...
pub fn delete_comment(range: TextRange, locator: &Locator) -> Edit {
...
pub fn remove_argument<T: Ranged>(
    argument: &T,
    arguments: &Arguments,
...
pub fn add_parameter(parameter: &str, parameters: &Parameters, source: &str) -> Edit {
...
pub fn pad_start(mut content: String, start: TextSize, locator: &Locator) -> String {
...
pub fn pad_end(mut content: String, end: TextSize, locator: &Locator) -> String {
    if locator
        .after(end)
        .chars()
        .next()
...
pub fn pad(content: String, range: TextRange, locator: &Locator) -> String {
    pad_start(
...
mod tests {
    ...
    use crate::edits::{
        make_redundant_alias, next_stmt_break, trailing_semicolon,
    };
...
"
    ...
    //     };
    //     let diag = {
    //         use ruff_codes::pycodestyle::rules::MissingNewlineAtEndOfFile;
