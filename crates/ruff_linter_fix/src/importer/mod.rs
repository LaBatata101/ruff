...
impl<'a> Importer<'a> {
    ...
    pub fn visit_type_checking_block(&mut self, type_checking_block: &'a Stmt) {
    ...
    /// import statement.
    pub fn add_import(&self, import: &NameImport, at: TextSize) -> Edit {
    ...
    /// of the file. Otherwise, it will be added after the most recent top-level
    /// import statement.
    pub fn runtime_import_edit(
        &self,
        import: &ImportedMembers,
        at: TextSize,
    ) -> Result<RuntimeImportEdit> {
        // Generate the modified import statement.
        let content = crate::codemods::retain_imports(
            &import.names,
            import.statement,
            self.locator,
            self.stylist,
    ...
    ///
    /// Attempts to reuse existing imports when possible.
    pub fn get_or_import_builtin_symbol(
        &self,
        symbol: &str,
        at: TextSize,
        semantic: &SemanticModel,
...
impl TypingImportEdit {
    pub fn into_edits(self) -> (Edit, Option<Edit>) {
