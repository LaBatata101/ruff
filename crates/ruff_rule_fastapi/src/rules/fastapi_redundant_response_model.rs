...
use ruff_linter_fix::edits::{remove_argument, Parentheses};
use ruff_linter_commons::helpers::is_fastapi_route_decorator;
...
/// FAST001
pub fn fastapi_redundant_response_model(checker: &CheckerSnapshot, function_def: &StmtFunctionDef) {
    ...
    for decorator in &function_def.decorator_list {
        let Some((call, response_model_arg)) =
            check_decorator(function_def, decorator, &semantic)
        else {
            continue;
        };
        let mut diagnostic =
