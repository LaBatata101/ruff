...
use ruff_codes::Rule;
use ruff_codes::rules::isort::{self, categorize::MatchSourceStrategy, ImportSection, ImportType};
...
fn is_first_party(import: &AnyImport, checker: &CheckerSnapshot) -> bool {
...
///
pub fn unused_import(checker: &CheckerSnapshot, scope: &Scope) {
    ...
    let dunder_all_exprs = find_dunder_all_exprs(&semantic);
...
fn fix_by_removing_imports<'a>(
    ...
    Ok(
        Fix::applicable_edit(edit, applicability).isolate(CheckerSnapshot::isolation(
...
fn fix_by_reexporting<'a>(
    ...
    let isolation = CheckerSnapshot::isolation(checker.semantic().parent_statement_id(node_id));
