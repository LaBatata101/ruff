...
/// UP024
pub fn os_error_alias_handlers(checker: &CheckerSnapshot, handlers: &[ExceptHandler]) {
...
/// UP024
pub fn os_error_alias_call(checker: &CheckerSnapshot, func: &Expr) {
...
/// UP024
pub fn os_error_alias_raise(checker: &CheckerSnapshot, expr: &Expr) {
    if matches!(expr, Expr::Name(_) | Expr::Attribute(_)) && is_alias(expr, &checker.semantic()) {
        atom_diagnostic(checker, expr);
    }
}
