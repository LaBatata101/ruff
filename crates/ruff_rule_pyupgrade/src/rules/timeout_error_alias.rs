...
fn atom_diagnostic(checker: &CheckerSnapshot, target: &Expr) {
...
fn tuple_diagnostic(checker: &CheckerSnapshot, tuple: &ast::ExprTuple, aliases: &[&Expr]) {
...
/// UP041
pub fn timeout_error_alias_handlers(checker: &CheckerSnapshot, handlers: &[ExceptHandler]) {
...
/// UP041
pub fn timeout_error_alias_call(checker: &CheckerSnapshot, func: &Expr) {
...
/// UP041
pub fn timeout_error_alias_raise(checker: &CheckerSnapshot, expr: &Expr) {
    if matches!(expr, Expr::Name(_) | Expr::Attribute(_)) && is_alias(expr, &checker.semantic(), checker.target_version()) {
        atom_diagnostic(checker, expr);
    }
}
