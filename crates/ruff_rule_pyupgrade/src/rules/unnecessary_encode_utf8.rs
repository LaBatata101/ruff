...
use ruff_linter_fix::edits::{pad, remove_argument, Parentheses};
...
fn match_encoding_arg(arguments: &Arguments) -> Option<EncodingArg> {
    match (&*arguments.args, &*arguments.keywords) {
        ...
        ([], [keyword]) => {
            if keyword.arg.as_ref().is_some_and(|arg| arg == "encoding") && is_utf8_encoding_arg(&keyword.value) {
...
/// UP012
pub fn unnecessary_encode_utf8(checker: &CheckerSnapshot, call: &ast::ExprCall) {
