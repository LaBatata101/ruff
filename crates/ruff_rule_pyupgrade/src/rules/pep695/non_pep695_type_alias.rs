...
use ruff_linter_commons::helpers::DisplayTypeVars;
...
use super::{
    expr_name_to_type_var, TypeParamKind, TypeVar, TypeVarReferenceVisitor,
};
...
/// UP040
pub fn non_pep695_type_alias_type(checker: &CheckerSnapshot, stmt: &StmtAssign) {
    ...
    let Some(vars) = type_params
        ...
        .map(|expr| {
            expr.as_name_expr().map(|name| {
                expr_name_to_type_var(&semantic, name).unwrap_or(TypeVar {
                    name: &name.id,
                    restriction: None,
...
/// UP040
pub fn non_pep695_type_alias(checker: &CheckerSnapshot, stmt: &StmtAnnAssign) {
    if checker.target_version() < PythonVersion::PY312 {
        return;
    }
