...
use crate::helpers::curly_escape;
...
fn simplify_conversion_flag(flags: CConversionFlags) -> String {
    ...
    if flags.intersects(CConversionFlags::BLANK_SIGN) && !flags.intersects(CConversionFlags::SIGN_CHAR) {
        flag_string.push(' ');
    }
    if flags.intersects(CConversionFlags::ZERO_PAD) && !flags.intersects(CConversionFlags::LEFT_ADJUST) {
        flag_string.push('0');
    }
    flag_string
}
...
fn clean_params_tuple<'a>(right: &Expr, locator: &Locator<'a>) -> Cow<'a, str> {
    if let Expr::Tuple(tuple) = &right {
        if tuple.len() == 1 && !locator.contains_line_break(right.range()) {
        ...
        for ast::DictItem { key, value } in items {
            if let Some(key) = key {
                ...
                }) = key
                {
                    ...
                    if is_multi_line && indent.is_none() {
...
/// UP031
pub fn printf_string_formatting(
