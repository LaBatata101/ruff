...
fn check_msg(checker: &CheckerSnapshot, msg: &Expr) {
    match msg {
...
fn check_log_record_attr_clash(checker: &CheckerSnapshot, extra: &Keyword) {
    match &extra.value {
        Expr::Dict(dict) => {
...
pub fn logging_call(checker: &CheckerSnapshot, call: &ast::ExprCall) {
    ...
    // G010
    if checker.enabled(Rule::LoggingWarn) && matches!(
            logging_call_type,
