...
/// AIR301
pub fn airflow_3_removal_expr(checker: &CheckerSnapshot, expr: &Expr) {
...
/// AIR301
pub fn airflow_3_removal_function_def(checker: &CheckerSnapshot, function_def: &StmtFunctionDef) {
    if !checker.semantic().seen_module(Modules::AIRFLOW) {
        return;
    }
...
///     pass
/// ```
fn check_function_parameters(checker: &CheckerSnapshot, function_def: &StmtFunctionDef) {
...
/// ```
fn check_call_arguments(checker: &CheckerSnapshot, qualified_name: &QualifiedName, arguments: &Arguments) {
...
/// info.dataset
/// ```
fn check_class_attribute(checker: &CheckerSnapshot, attribute_expr: &ExprAttribute) {
    let ExprAttribute { value, attr, .. } = attribute_expr;

    let semantic = checker.semantic();
    let Some(qualname) = typing::resolve_assignment(value, &semantic) else {
        return;
    };
...
///     context.get("conf")  # 'conf' is removed in Airflow 3.0
/// ```
fn check_context_key_usage_in_call(checker: &CheckerSnapshot, call_expr: &ExprCall) {
...
fn check_context_key_usage_in_subscript(checker: &CheckerSnapshot, subscript: &ExprSubscript) {
    if !in_airflow_task_function(&checker.semantic()) {
        return;
    }
...
/// ```
fn check_method(checker: &CheckerSnapshot, call_expr: &ExprCall) {
...
/// SubDagOperator()
/// ```
fn check_name(checker: &CheckerSnapshot, expr: &Expr, range: TextRange) {
    let semantic = checker.semantic();
